...
func Assert(b bool) {
	if !b { panic("Assertion Error") }
}

// Equals uses multiple methods of testing equality between two
...
func ContainsObject(slice interface{}, contains interface{}) bool {
	...
	for i := 0; i < reflectedSlice.Len(); i++ {
		if Equals(reflectedSlice.Index(i).Interface(), contains) {
			return true
		}
	}
	return false
...
// Count counts the occurrences of the value in the slice
func Count(slice interface{}, elem interface{}) int {
	count := 0
	reflectedSlice := reflect.ValueOf(slice)
	for i := 0; i < reflectedSlice.Len(); i++ {
		if Equals(reflectedSlice.Index(i).Interface(), elem) {
			count += 1
		}
	}
	return count
...
func MinSlice(slice interface{}) interface{} {
	reflectedSlice := reflect.ValueOf(slice)
	if reflectedSlice.Len() == 0 {
		panic("MinSlice() arg is an empty slice")
	}
	min := reflectedSlice.Index(0).Interface()
	for i := 0; i < reflectedSlice.Len(); i++ {
		elem := reflectedSlice.Index(i).Interface()
		switch min.(type) {
			case int:
				if elem.(int) < min.(int) {
					min = elem
			...
			case string:
				if elem.(string) < min.(string) {
					min = elem
				}
			case float64:
				if elem.(float64) < min.(float64) {
					min = elem
				}
		}
	}
...
func MaxSlice(slice interface{}) interface{} {
	...
	max := reflectedSlice.Index(0).Interface()
	for i := 0; i < reflectedSlice.Len(); i++ {
		elem := reflectedSlice.Index(i).Interface()
		switch max.(type) {
			case int:
			...
			case string:
				if elem.(string) > max.(string) {
					max = elem
				}
			case float64:
				if elem.(float64) > max.(float64) {
					max = elem
				}
		}
	}
